...
exports.getNews = asyncHandler(async (req, res, next) => {
  const news = await News.find()
    .sort({ Ognoo: +1 })
    .populate("journalist", "name imageUrl")
...
exports.getNewsByUrl = asyncHandler(async (req, res, next) => {
  const news = await News.findOne({ uniqueUrl: req.params.url }).populate(
    "journalist",
    "name imageUrl"
  );
  if (!news) {
...
exports.updateNewsById = asyncHandler(async (req, res, next) => {
  ...
  /* check redakts admin */
  if (
    (news.journalist.toString() !== req.userId && req.userRole !== "Redakts") ||
    req.userRole !== "Redakts" ||
    req.userRole !== "Admin"
  ) {
    res.status(401).json({
      success: false,
...
exports.deleteNews = asyncHandler(async (req, res, next) => {
  ...
  if (
    (news.journalist.toString() !== req.userId && req.userRole !== "Admin") ||
    (req.userRole !== "Redakts" && req.userRole !== "Admin")
  ) {
    throw new MyError("Таны бичсэн мэдээ биш байна !!!", 400);
  }
  /* ustgagdsan medeelliin zurgiig ustgah  */
  // if(news.image && news.image.public_id)
  let image = await cloudinary.uploader.destroy(news.image.public_id);
  news.remove();
